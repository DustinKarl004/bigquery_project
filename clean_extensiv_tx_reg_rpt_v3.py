...
# Preprocess the "QtyIn" column
def preprocess_qty(value):
    try:
        return int(value)  # Attempt to convert to integer
    except ValueError:
        logging.warning(f"Non-integer value found in 'QtyIn': {value}")
        return None  # Return None for non-integer values

# Preprocess the "QtyOut" column
def preprocess_qty_out(value):
    try:
        return int(value)  # Attempt to convert to integer
    except ValueError:
        logging.warning(f"Non-integer value found in 'QtyOut': {value}")
        return None  # Return None for non-integer values

try:
    # Open the CSV file and process it line by line
    with open('ExtensivTxRegRpt_week1.csv', mode='r', encoding='utf-8') as csvfile:
        reader = csv.reader(csvfile)
        headers = next(reader)  # Read the header row
        logging.info(f"Headers found: {headers}")  # Log the headers for debugging

        # Check if "textbox69" is in the headers
        if "textbox69" not in headers:
            logging.warning("'textbox69' column not found in the CSV file.")
        else:
            textbox69_index = headers.index("textbox69")  # Get the index of "textbox69"
                ...
                try:
                    ...
                    for col_idx, value in enumerate(row):
                        ...
                        if headers[col_idx] == "QtyIn":
                            cleaned_value = preprocess_qty(cleaned_value)  # Use the preprocessing function
                        
                        # Check for "QtyOut" column and ensure it's an integer
                        if headers[col_idx] == "QtyOut":
                            cleaned_value = preprocess_qty_out(cleaned_value)  # Use the preprocessing function
                        ...
                        if headers[col_idx] == "textbox69":
                            try:
                                ...
                                logging.error(f"Unexpected error while processing line {row_num}: {str(e)}")
                                cleaned_value = None  # Set to None for any unexpected errors
    ...
    if len(problematic_rows) > 0:
        ...
        raise Exception("CSV processing encountered too many errors, giving up.")

    # Preprocess the "textbox69" column
    def preprocess_textbox69(value):
        try:
            return int(value)  # Attempt to convert to integer
        except ValueError:
            logging.warning(f"Non-integer value found in 'textbox69': {value}")
            return None  # Return None for non-integer values

    # Apply preprocessing to the "textbox69" column
    if 'textbox69' in df.columns:
        df['textbox69'] = df['textbox69'].apply(preprocess_textbox69)

    # After processing the DataFrame
    if 'QtyIn' in df.columns:
        df['QtyIn'] = df['QtyIn'].apply(preprocess_qty)

    if 'QtyOut' in df.columns:
        df['QtyOut'] = df['QtyOut'].apply(preprocess_qty_out)
