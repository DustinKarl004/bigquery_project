...
try:
    ...
    # Clean the OrderId column
    if 'OrderId' in df.columns:
        # Convert OrderId to numeric, coercing errors to NaN
        df['OrderId'] = pd.to_numeric(df['OrderId'], errors='coerce')
        
        # Log rows with invalid OrderId values
        invalid_order_ids = df[df['OrderId'].isna()]
        if not invalid_order_ids.empty:
            logging.warning(f"Found {len(invalid_order_ids)} rows with invalid OrderId values.")
            for index, row in invalid_order_ids.iterrows():
                logging.warning(f"Row {index + 2}: OrderId = {row['OrderId']} (invalid)")

        # Drop rows with invalid OrderId values
